...
public class UniquePaths {
    ...
    /*********************************2 D Array 2********************************************/
    //The space complexity of the optimized solution is O(m * n) because we use a 2D array memo of size m * n
    ...
    private static int uniquePaths(int m, int n , int [][] memo) {
        ...
        return memo[m-1][n-1] = uniquePaths(m - 1, n,memo) + uniquePaths(m, n - 1,memo);
    ...
    private static int uniquePaths1(int m, int n , Map<String,Integer> memo) {
        ...
        if(m < 1 || n < 1) return 0;
        String key = m +"-"+n;
        if(!memo.containsKey(key)) memo.put(key,uniquePaths1(m - 1, n,memo) + uniquePaths1(m, n - 1,memo));
        return memo.get(key);
    }
