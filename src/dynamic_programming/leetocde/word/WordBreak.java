...
public class WordBreak {
    ...
    /*   String s= "catsandog";
        ...
        System.out.println(wordBreakBottomUp(s, wordDict));
     ...
     * space(m2)
     */
    public static boolean wordBreakTopDown(String s, List<String> wordDict, Map<String, Boolean> memo) {
        if (s.isEmpty()) {
            return true;
        }
        ...
        for (String word : wordDict) {
            if (s.startsWith(word)) {
                if (wordBreakTopDown(s.substring(word.length()), wordDict, memo)) {
                    memo.put(s, true);
                    return true;
                }
            }
